...
const ArtistSpeedDial: FC<{ artist: Artist }> = ({ artist }) => {
    ...
    const speedDialActions: SpeedDialAction[] = [
        ...(artist.linkTwitter
            ? [
                { icon: <Avatar src={twitterIcon} alt="Twitter" sx={{ width: 52, height: 52 }} />, name: 'Twitter', href: artist.linkTwitter },
                { icon: <Avatar src={nitterIcon} alt="Nitter" sx={{ width: 52, height: 52 }} />, name: 'Nitter', href: artist.linkTwitter.replace('x.com', 'nitter.net') },
            ]
            : []),
        ...(artist.linkPixiv
            ? [
                { icon: <Avatar src={pixivIcon} alt="Pixiv" sx={{ width: 52, height: 52 }} />, name: 'Pixiv', href: artist.linkPixiv },
            ]
            : []),
    ];
